...
class SearchView(FormView):
    ...
    def faceted_search(facets, query, search_queryset, request) -> set:
        ...
        # no repetitions)
        return set(
            map(lambda x: int(x), (search_queryset.values_list("pk", flat=True)))
        )
    ...
    def content_search(request, codes):
        ...
        file_id_set = set(SymbolicMusicFile.objects.values_list("id", flat=True))

        if any(key in codes for key in request.GET):
            for key, value in request.GET.lists():
                if key in codes:
                    min_value, max_value = value[0].split(",")
                    single_feature_results = single_feature_search(
    ...
    def get(self, request, *args, **kwargs):
        ...
        merged_result_ids = faceted_search_results.intersection(content_search_results)

        files_queryset = SymbolicMusicFile.objects.filter(id__in=merged_result_ids)
        files = []
        ...
        self.search_queryset = (
            SearchQuerySet().models(SymbolicMusicFile).filter(django_id__in=ids_for_sqs)
        )

        context = {
            "content_search_form": ContentSearchForm(
                feature_types=self.feature_types, file_ids=ids_for_sqs, data=request.GET
            ),
