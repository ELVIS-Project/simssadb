...
from database.forms.creation_forms import (
...
class CreationView(FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, contribution_form=contribution_forms)
        )
    ...
    def form_valid(self, form, contribution_forms, request):
        ...
        work = MusicalWork(variant_titles=titles, _sacred_or_secular=sacred_or_secular)
        work.save()
        ...
        for form in contribution_forms:
            ...
            person, created = Person.objects.get_or_create(
                given_name=person_given_name, surname=person_surname
            )
            if range_date_birth:
                birth_year_from = form.cleaned_data["person_range_date_birth"].lower
                if birth_year_from:
                    birth_date_from = datetime.date(birth_year_from, 1, 1)
                else:
                    birth_date_from = None
                birth_year_to = form.cleaned_data["person_range_date_birth"].upper
                if birth_year_to:
            ...
            if range_date_death:
                death_year_from = form.cleaned_data["person_range_date_death"].lower
                if death_year_from:
                    death_date_from = datetime.date(death_year_from, 1, 1)
                else:
                    death_date_from = None
                death_year_to = form.cleaned_data["person_range_date_death"].upper
                if death_year_to:
    ...
    def form_invalid(self, form, contribution_forms):
        ...
        """
        return self.render_to_response(
            self.get_context_data(form=form, contribution_forms=contribution_forms)
        )
