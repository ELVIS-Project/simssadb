...
from database.forms.creation_forms import FileForm
...
from database.models import (Source, File, #SymbolicMusicFile,
                             Software)#Encoder)
...
class FileCreationView(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        child_source_form = Source(request.POST, # again fix prefix stuff 
                                                    prefix='child')
        parent_source_form = Source(request.POST,
                                                     prefix='parent')
    ...
    def form_valid(self, work_formset, section_formset, part_formset,
        ...
        if parent_source_title:
            ...
            parent_collection = Source(title=parent_source_title, # collection stuff fix
        ...
        child_collection = Source(title=child_source_title,
        ...
        for form in work_formset:
            ...
            if not software: software = None # is this necessary fix
            ...
            file = File(file=file, manifests=instantiation, # this def needs fixing for attributes
                                     file_type=file_type, file_size=size,
                                     encoding_date=encoding_date,
                                     encoded_with=software)
            file.save()
        ...
        for form in section_formset:
            ...
            #    else:
               #     encoder = None
                ...
                file = File(file=file, manifests=instantiation,
                                         file_type=file_type, file_size=size,
                                         encoding_date=encoding_date,
                                         encoded_with=software)
                file.save()
