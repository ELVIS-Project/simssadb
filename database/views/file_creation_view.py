...
from database.models import Source, CollectionOfSources, SymbolicMusicFile, Encoder
...
class FileCreationView(FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if work_queryset.exists():
            ...
            DynamicSectionFileForm = type(
                "SectionFileForm", (FileForm,), {"section": section_field}
            )
            part_field = forms.ModelChoiceField(queryset=parts)
            DynamicPartFileForm = type(
                "PartFileForm", (FileForm,), {"part": part_field}
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if work_queryset.exists():
            ...
            WorkFormSet = formset_factory(FileForm)
            work_formset = WorkFormSet(request.POST, request.FILES, prefix="work")
            if sections.exists():
                section_field = forms.ModelChoiceField(queryset=sections)
                DynamicSectionFileForm = type(
                    "SectionFileForm", (FileForm,), {"section": section_field}
                ...
                section_formset = SectionFormSet(
                    request.POST, request.FILES, prefix="section"
                )
            else:
                section_formset = None
            ...
            if parts.exists():
                ...
                PartFormSet = formset_factory(DynamicPartFileForm)
                part_formset = PartFormSet(request.POST, request.FILES, prefix="part")
            else:
                part_formset = None
        ...
        child_source_form = CollectionOfSourcesForm(request.POST, prefix="child")
        parent_source_form = CollectionOfSourcesForm(request.POST, prefix="parent")

        # Forms must be valid but section_formset or part_formset can be None
        if (
            work_formset.is_valid()
            and (section_formset is None or section_formset.is_valid())
            and (part_formset is None or part_formset.is_valid())
            and (child_source_form.is_valid() and parent_source_form.is_valid())
        ):
    ...
    def form_valid(
        ...
        if parent_source_title:
            parent_collection_url = parent_source_form.cleaned_data["collection_url"]
            parent_archive = parent_source_form.cleaned_data["archive"]
            parent_portions = parent_source_form.cleaned_data["portions"]

            parent_collection = CollectionOfSources(
                title=parent_source_title, url=parent_collection_url
            )
        ...
        child_collection = CollectionOfSources(
            title=child_source_title, url=child_collection_url
        )
        ...
        for form in work_formset:
            ...
            instantiation = SourceInstantiation(source=child_source, work=work)
            instantiation.save()

            file = SymbolicMusicFile(
                file=file,
                manifests=instantiation,
                file_type=file_type,
                file_size=size,
