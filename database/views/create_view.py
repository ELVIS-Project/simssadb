...
class CreateMusicalWorkViewCustom(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for key, value in post_dict.lists():
            if key.startswith("title") and not (
                key.startswith("title_section") or key.startswith("title_source")
            ):
        ...
        for index in range(1, 4):
            if post_dict.get("given_name" + str(index), False) or post_dict.get(
                "person_selected" + str(index), False
            ):
    ...
    def _parse_contribution(self, post_dict, index, work):
        ...
        if person_id:
            ...
            birth_start_date_text = post_dict.get(
                "range_date_birth" + str(index) + "_0", False
            )
            if birth_start_date_text:
                start_date_birth = datetime.datetime.strptime(
                    birth_start_date_text, "%Y"
                ).date()
            else:
                start_date_birth = None
            birth_end_date_text = post_dict.get(
                "range_date_birth" + str(index) + "_0", False
            )
            if birth_end_date_text:
                end_date_birth = datetime.datetime.strptime(
                    birth_end_date_text, "%Y"
                ).date()
            else:
                end_date_birth = None
            ...
            if death_start_date_text:
                start_date_death = datetime.datetime.strptime(
                    death_start_date_text, "%Y"
                ).date()
            else:
                start_date_death = None
            death_end_date_text = post_dict.get(
                "range_date_death" + str(index) + "_0", False
            )
            if death_end_date_text:
                end_date_death = datetime.datetime.strptime(
                    death_end_date_text, "%Y"
                ).date()
            else:
                end_date_death = None
        ...
        if location_id:
            ...
            location = None
        certainty_string = post_dict.get(
            "certainty_of_attribution" + str(index), "false"
        )
        ...
        contribution = self._create_contribution(
            start_date_birth, end_date_birth, role, person, certainty, location, work
        )
        return contribution
    ...
    def _create_musical_work(variant_titles, sacred_or_secular) -> MusicalWork:
        work = MusicalWork(
            variant_titles=variant_titles, _sacred_or_secular=sacred_or_secular
        )
        return work

    @staticmethod
    def _create_contribution(
        start_date, end_date, role, person, certainty, location, work
    ):
        if start_date:
    ...
    def _create_person(given_name, surname, date_of_birth, date_of_death):
        person = Person(
            ...
            surname=surname,
            range_date_birth=date_of_birth,
            range_date_death=date_of_death,
        )
        return person
    ...
    def _create_section(title, musical_work, ordering):
        section = Section(title=title, musical_work=musical_work, ordering=ordering)
        return section
