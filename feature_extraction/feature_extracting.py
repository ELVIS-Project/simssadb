...
def conversion(
    ...
    parsable_format = [
        ...
        ".mei",
        ".xml",
    ]  # We can test out MuseData as well
    print(path)
    ...
    if os.path.exists(conversion_file_path) is False:
        os.mkdir(conversion_file_path)
    file_name, extension = os.path.splitext(filename_w_ext)
    if extension.lower() == ".mid" or extension.lower() == ".midi":
        num_of_midi_file += 1
        num_of_midi_file_feature, extracted = extract_features(
            ftotal,
            jar_file,
            config_file,
            path,
        ...
        try:
            ...
            num_of_converted_files_feature, extracted = extract_features(
                ftotal,
                jar_file,
                config_file,
                new_path,
        ...
        print(
            path
            + " is not convertible by music21. Therefore it cannot be processed by jsymbolic",
            file=flog,
        )
...
def standard_output(
    ...
    print(
        ...
        num_of_total_files,
        "files,",
        num_of_total_files - num_of_non_processed_files,
        "are processed.",
        num_of_non_processed_files,
        "are not processed since their formats are not suppoted.",
    )
    print(
        "Among",
        num_of_total_files - num_of_non_processed_files,
...
def extract_features_setup(jar_file, config_file, path, feature_path=""):
    ...
    num_of_midi_file_feature = (
        0
    )  # The number of midi files whose features are extracted successfully
    num_of_converted_files = (
        0
    )  # The number of processed files that are converted successfully
    num_of_converted_files_feature = (
        0
    )  # The number of processed files that are converted
    ...
    if os.path.isdir(path):  # The path is a folder
        ...
        ftotal = open(
            os.path.join(path, "standard_output_log_batch" + time + ".txt"), "w"
        )
        flog = open(
            os.path.join(path, "conversion_error_log_batch" + time + ".txt"), "w"
        )
        if feature_path == "":
            feature_path = os.path.join(
                path, "extracted_features"
            )  # When doing on a folder, this function will create a separate folder
        if os.path.exists(feature_path) is False:
            os.mkdir(feature_path)
        for id, fn in enumerate(os.listdir(path)):
            if (
                fn.find(".DS_Store") == -1
                and fn.find("extracted_features") == -1
                and fn.find("conversion") == -1
                and fn.find("log") == -1
            ):  # Only convert the files that are already there
        ...
        flog = open(
            os.path.join(os.path.dirname(path), "conversion_error_log.txt"), "a"
        )
        ftotal = open(
            os.path.join(os.path.dirname(path), "standard_output_log.txt"), "a"
        )
        if feature_path == "":
            feature_path = os.path.join(
                os.path.dirname(path), "extracted_features"
            )  # Use the file path as the feature path
        if os.path.exists(feature_path) is False:
            os.mkdir(feature_path)
        ...
        print(
            "The path you specified might not exist. Please specify a valid path, either a folder or a file!"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=os.path.join(os.getcwd(), "jSymbolic_2_2_user", "jSymbolic2.jar"),
    )
    parser.add_argument(
        "-c",
        ...
        default=os.path.join(
            os.getcwd(), "jSymbolic_2_2_user", "jSymbolicDefaultConfigs.txt"
        ),
    )
    parser.add_argument(
        "-p",
    ...
    extracted = extract_features_setup(
        args.jsymbolic_file, args.jsymbolic_config_file, args.path
    )
    print(extracted)
